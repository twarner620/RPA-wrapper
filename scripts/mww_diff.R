...
option_list = list(
    ...
    make_option(c("-F", "--freq2"), type="character", default=NULL, 
              help="annotated file generated by bedtools annotate", metavar="filetype"),
...
df1<-read.table(opt$freq, sep="\t", header =T,row.names=1, quote="", check.names=FALSE)
order=read.table(opt$meta, row.names=opt$names, sep="\t",header = F, quote="")
df2<-read.table(opt$freq2, sep="\t", header =T,row.names=1, quote="", check.names=FALSE)

#3.Calculating Mann whitney P-values
writeLines("\n...Calculating pvalues with baseline reference...\n")
groups=order[rownames(df1),][,opt$groupcol]
head=c("Cellline",colnames(df1))
#For all
add=c("all")
